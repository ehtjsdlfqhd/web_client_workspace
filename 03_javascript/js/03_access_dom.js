...
// html문서의 계층구조를 반영해 javascript객체로 변환
// document
...
function getById(){
    ...
    const notExist = document.getElementById("dfefewer");
...
/**
 * document.getElementsByClssName(class: string): object[]
 */
function getByClass(){
...
function checkAll(){
    ...
    const hobbies = document.getElementsByName("hobby"); //name=hobby인 객체 가져옴
    const all = document.getElementById("all")
    // id가 유일하기 때문에 해당 아이디를 변수처럼 바로 사용
...
/**
 * document.querySelector(selector: string): HTMLElemnet | null
 * css선택자를 이용해서 하나의 DOM 요소 반환
...
function _querySelectorAll(){
    ...
    console.log(list);
    // list.innerHTML = 'xxxxx' 배열객체에는 innerHTML 속성이 있다.
    for(let i = 0; i < list.length; i++){
